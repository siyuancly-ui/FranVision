...
```bash
cd photo-sync-worker
npm test           # node --test, 62 cases, NO network
```
...
| `src/supabase.js` | Raw-fetch: Storage upload (`x-upsert`), PostgREST `/rpc/*`, `photo_sync_state` read/write incl. the lease lock (one conditional `PATCH`), `video_sync_pending` insert/list/delete, and `getProjectVideo` (reads one video's current record off `projects.data.videos[]`, used before a delete to find the `streamUid` to free) |
