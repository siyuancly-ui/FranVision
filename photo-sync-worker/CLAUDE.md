...
```bash
cd photo-sync-worker
npm test           # node --test, 65 cases, NO network
```
...
| `src/paths.js` | Pure path parsing: job folder / sub-folder / relative path / extension, `SYNC_FOLDERS`/`VIDEO_SYNC_FOLDERS` matching, the `MLS for download` loop-guard, `downloadCopyPath`, `parseAddressFromJobFolder` (interim address extraction, see below) |
