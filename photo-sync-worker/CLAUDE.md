...
```bash
cd photo-sync-worker
npm test           # node --test, 57 cases, NO network
```
...
- **`SYNC_FOLDERS` = `MLS, HDR Photos, Virtual Staging, Floorplan, Local Report`.** Only web-image extensions (`.jpg/.jpeg/.png/.webp`) under these sub-folders of a tagged job folder are synced. `0 RAW`, `Home Report`, videos, PDFs, `MLS for download` itself (loop guard) are all ignored. `DOWNLOAD_SET_FOLDERS` = `MLS, HDR Photos`. **Both `MLS` and `HDR Photos` are listed deliberately** — job-generator renamed its always-present high-res-photos folder from `MLS` to `HDR Photos` on 2026-09-12 (pure naming change on that side, see `job-generator/CLAUDE.md`'s "MLS renamed to HDR Photos" decision), but jobs created before that rename still have a real `MLS` folder on Dropbox. Listing both here means neither old nor new jobs stop syncing. `DOWNLOAD_SUBFOLDER` (`MLS for download`) is a separate, unrelated name — Photo Sync Worker's own derived-copy folder, not touched by that rename. (This was briefly broken 2026-09-08→2026-09-14: the deployed config only listed `MLS`, so every post-rename job's main photos silently never synced — fixed in commit `16e8562`, verify against the live `wrangler.jsonc`/`GET /admin/status` rather than trusting this doc if something seems off again.)
