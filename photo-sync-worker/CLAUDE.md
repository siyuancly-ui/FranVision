...
```bash
cd photo-sync-worker
npm test           # node --test, 86 cases, NO network
```
...
- **`LARGE_THUMB_FOLDERS` = `Cover, Closing, Callout, Local Report` (2026-09-15), a second render for [[franvision-delivery-page]]'s full-bleed sections.** Every `processPhotoBatch` upsert whose `subFolder` matches this list gets a SECOND `get_thumbnail_batch` call at `DOWNLOAD_THUMB_SIZE` (reusing the existing w2048h1536 config, same as the Dropbox delivery-copy render — independent pass, own batch/fallback, not shared bytes) uploaded to Supabase Storage as `<photoId>_large.jpg` (`sb.uploadLarge`), then a follow-up `photos_upsert` merges `hasLarge:true` onto the already-saved record. **Deliberately NOT applied to the main gallery (`MLS`/`HDR Photos`)** — those stay small-only (`THUMB_SIZE`/`_thumb.jpg`) to avoid inflating storage and Feature-Sheet-Builder-picker load time for every synced photo, not just the few that actually render full-bleed on the delivery page. Consequence: when a Job has no `Cover`/`Closing` override (see the SYNC_FOLDERS note above), delivery-page's hero/closing fallback uses the SMALL gallery thumb and will look softer than it would with an override — an intentional nudge toward using the override folders, not a bug to fix by also large-rendering the whole gallery. Entirely best-effort (`large_batch_failed`/`large_render_failed`), never blocks the small thumb or the rest of the batch.
