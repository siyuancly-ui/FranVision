...
```bash
cd photo-sync-worker
npm test           # node --test, 72 cases, NO network
```
...
| `src/supabase.js` | Raw-fetch: Storage upload (`uploadThumb` + `uploadLarge`, both `x-upsert`), PostgREST `/rpc/*`, `photo_sync_state` read/write incl. the lease lock (one conditional `PATCH`), `video_sync_pending` insert/list/delete, and `getProjectVideo` (reads one video's current record off `projects.data.videos[]`, used before a delete to find the `streamUid` to free) |
...
- **`SYNC_FOLDERS` = `MLS, HDR Photos, Virtual Staging, Floorplan, Local Report, Cover Photo, Closing Photo, Drone Callout`.** Only web-image extensions (`.jpg/.jpeg/.png/.webp`) under these sub-folders of a tagged job folder are synced. `0 RAW`, `Home Report`, videos, PDFs, `MLS for download` itself (loop guard) are all ignored. `DOWNLOAD_SET_FOLDERS` = `MLS, HDR Photos`. Both `MLS` and `HDR Photos` are listed because Job Generator renamed its `MLS` folder to `HDR Photos` on 2026-09-12 (pure naming change) — pre-rename jobs still have a real `MLS` folder on Dropbox, new jobs use `HDR Photos`; both need to keep syncing. (This was briefly broken 2026-09-08→2026-09-14: the deployed config only listed `MLS`, so every post-rename job's main photos silently never synced — fixed in commit `16e8562`, verify against the live `wrangler.jsonc`/`GET /admin/status` rather than trusting this doc if something seems off again.) **`Cover Photo` / `Closing Photo` / `Drone Callout` (2026-09-15) are delivery-page's manual-override folders** — Job Generator doesn't create these by default yet (deferred on purpose, see [[franvision-delivery-page]]), so they only ever get anything in them if a human manually creates the folder in Dropbox and drops a photo in — this Worker doesn't care either way, it syncs whatever's there under these names exactly like any other folder.
- **A recognized folder can be nested inside another one, not just directly under the job folder (confirmed 2026-09-16)** — e.g. `Drone Callout` living at `<job>/HDR Photos/Drone Callout/x.jpg` rather than `<job>/Drone Callout/x.jpg`. `paths.js#parseJobPath` now returns `ancestors` (every directory name between the job folder and the file); `matchAncestorFolder(ancestors, list)` scans INNERMOST-first so the most specific match wins (e.g. `Drone Callout` over the outer `HDR Photos`), and `classifyForSync`/`classifyForVideoSync` use that matched name — not always `parsed.subFolder` — as the `folder` value written to Supabase. Fully backward compatible: a plain, non-nested file only ever has one ancestor, so existing behavior for every current flat folder is unchanged; an unrecognized inner folder (e.g. a photographer's own `HDR Photos/Retouched/x.jpg` organizing habit) falls through to the outer recognized folder exactly as before.
