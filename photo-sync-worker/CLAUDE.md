...
```bash
cd photo-sync-worker
npm test           # node --test, 97 cases, NO network
```
...
| `src/paths.js` | Pure path parsing: job folder / sub-folder / `ancestors` (every directory between job folder and file) / relative path / extension, `matchAncestorFolder` (innermost-first folder-name matching, so a recognized folder can nest inside another one — see below) for `SYNC_FOLDERS`/`VIDEO_SYNC_FOLDERS`, the `MLS for download` loop-guard, `downloadCopyPath`, `parseAddressFromJobFolder` (interim address extraction, see below) |
