...
--   * paid / unlocked  OWNED BY THE ADMIN SIDE (delivery-page /admin) -- this
--                    function never touches them, so re-running Create/Update Job
...
create or replace function public.jg_delivery_hub(p_token text, p_job_id text, p_lines jsonb, p_wave_view_url text, p_total_cents integer)
...
begin
  ...
  insert into public.delivery_hub (job_id, token, lines, wave_view_url, total_cents)
  values (p_job_id, encode(extensions.gen_random_bytes(16), 'hex'), p_lines, nullif(btrim(p_wave_view_url), ''), p_total_cents)
...
revoke all on function public.jg_delivery_hub(text, text, jsonb, text, integer) from public;
grant execute on function public.jg_delivery_hub(text, text, jsonb, text, integer) to anon;
