...
async function generateDeliveryEmails({ jobId, jobFolderPath, folderName, clientName, address, order, componentFolders, totalCents, preTaxCents, client, waveViewUrl, getGalleryToken, saveDeliveryHub }) {
  try {
    ...
    if (jobId && typeof saveDeliveryHub === 'function') {
      ...
      try { hubToken = (await saveDeliveryHub({ jobId, lines: hubLines, waveViewUrl: waveViewUrl || null, totalCents })) || null; } catch (err) { hubToken = null; }
    }
