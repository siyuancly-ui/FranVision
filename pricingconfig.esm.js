...
// ---------------------------------------------------------------------
// ES Module version. Same rules and prices as pricing-config.js, only the
// module wrapper changed (UMD -> plain `export`) so this can be imported
// directly in Node (with "type": "module" in package.json, or a .mjs
// file) or in a browser via <script type="module">, with no dual-wrapper
// translation step needed when reusing this file elsewhere (e.g. Wix
// Velo, which also expects ES Module syntax).
//
// This file does NOT replace pricing-config.js -- both exist side by side.
// pricing-config.js keeps working as-is with the existing UMD-based
// tester.html / engine.test.js setup.
// ---------------------------------------------------------------------
