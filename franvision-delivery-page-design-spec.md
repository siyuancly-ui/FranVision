...
7. **Neighborhood report** (Schools / Parks & Rec / Transit / Safety) — four content blocks, each with a colored section header, icon, short intro paragraph, and a structured list of nearby amenities with distances/walk times.
   - **Confirmed 2026-09-15: this is the existing "Local Report" folder from the job-generator folder structure**, not a separate integration — Franky's team drops a PDF or image-format report into each Job's `Local Report` folder. **Format implication (not yet resolved):** [[franvision-photo-sync-worker]]'s photo sync already includes `Local Report` in `SYNC_FOLDERS` (it was there even before the 2026-09-14 HDR Photos fix), so an image-format report would already flow through the existing Dropbox-thumbnail-API pipeline into Supabase like any other photo — but a PDF would not (Dropbox's thumbnail endpoint is image-only, and nothing today extracts or renders PDF pages). Confirm with Franky whether Local Report is consistently one format or varies per job before assuming either path works.
