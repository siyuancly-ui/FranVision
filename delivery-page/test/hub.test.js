...
test('deliver-first (unlocked but NOT paid): downloads open, but the invoice button stays "View invoice & pay" with its dialog and the copy says payment is still due', () => {
  ...
  assert.match(html, /Payment is still due/);
  assert.doesNotMatch(html, /Invoice &amp; receipt|Payment received/);
});
...
test('dialog splits invoice from payment: View invoice (PDF) first, then Pay by credit card (Wave page) and Pay by e-Transfer; each only when it exists', () => {
  ...
  assert.match(full, /class="hub-pay is-ghost" id="hubView" href="\/deliver\/x\/y\/invoice" target="_blank" rel="noopener">View invoice/);   // our invoice page (PDF shown, not force-downloaded)
  ...
  assert.match(noPdf, /class="hub-pay is-ghost" id="hubView" href="https:\/\/next\.waveapps\.com\/pay\/abc"/);   // no PDF link yet -> the Wave page shows the invoice
